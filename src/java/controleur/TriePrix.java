 ...
 * @author itu
 */
public class TriePrix extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet TriePrix</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet TriePrix at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            PrixPoketra[] prixPoketra = new PrixPoketra().selectminmax(Connexion.connect(),min,max);
            request.setAttribute("prixPoketra", prixPoketra);
        } catch (Exception ex) {
            ex.printStackTrace();
        } 
